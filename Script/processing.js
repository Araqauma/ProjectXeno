...
class Sketch {
    ...
    createCanvas(width, height) {
        ...
        if (width == undefined) {
            if (canvasWidth != undefined) width = canvasWidth;
            else width = 700;

            if (canvasHeight != undefined) height = canvasHeight;
            else height = 700;
        }
...
window.onload = function() {
    if (typeof setup === 'function') {
        sketch.removeCanvas();
        setup();
        if (canvasWidth != undefined) canvasWidth = sketch.width;
        if (canvasHeight != undefined) canvasHeight = sketch.height;
    } else {
        sketch.createCanvas();
    }
};
